...
from config import DASHBOARD_FETCH_LIMIT, LIVE_BOARD_FETCH_LIMIT, TEST_ROOM_NAME, UI_FONT_FAMILY
    ...
    if not opinion_df.empty:
        ...
        def do_toggle_comment_like(comment_id):
            if is_data_collection_frozen(room_name):
                st.toast(f"🔒 데이터 수집 기간이 종료되어 좋아요를 변경할 수 없습니다. 테스트는 '{TEST_ROOM_NAME}'에서 해주세요.", icon="🔒")
                return
